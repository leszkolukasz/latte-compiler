...
data TopDef' a
    = TopFnDef a (FnDef' a)
    | TopClassDef a Ident [ClassMember' a]
    | TopClassDefWithInher a Ident Ident [ClassMember' a]
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)
...
data ClassMember' a = Field a (Type' a) Ident | Method a (FnDef' a)
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)
...
instance HasPosition TopDef where
  hasPosition = \case
    TopFnDef p _ -> p
    TopClassDef p _ _ -> p
    TopClassDefWithInher p _ _ _ -> p
